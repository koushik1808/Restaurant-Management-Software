    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg modal-fullscreen-md-down">
      <div class="modal-content">
          ...
          <div class="table-responsive">
            <table class="table table-striped">
              ...
              <tbody>
                ...
                <tr>
                    <form action="{{route('Admin.count_add_item')}}" method="POST">
                      @csrf
                  <td>{{$i=$i+1}}</td>
                  <td>{{$item->	manu_code}}</td>
                  <td>{{$item->manu}}</td>
                  <td class="d-flex gap-2 h-100 p-4">
                    <a href="{{route('Admin.count_sub',['id'=>$item->id])}}" class="btn btn-secondary"><i
                        class='bx bx-minus'></i></a>

                    <input type="text" name="count" class="form-control w-25" value="{{$item->count}}" />
                    <input type="text" hidden name="id" class="form-control w-25" value="{{$item->id}}" />
        ...
        <div class="modal-footer">
          ...
          @else
          <a href="{{route('User.Dashbroad')}}" class="btn btn-success">Checkout</a>
          @endif
